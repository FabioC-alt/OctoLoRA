...
for i in "${!VARIANT_NAMES[@]}"; do
    ...
    train_job=$(sbatch --parsable "${dep_arg[@]}" scripts/submit_octolora.sh $flags)
    echo "  -> train job $train_job"

    echo "Submitting evaluation for '$name', after job $train_job"
    if [[ -n "$N_EXAMPLES" ]]; then
        eval_job=$(sbatch --parsable --dependency=afterok:"$train_job" scripts/submit_evaluate.sh "results/$name" "$N_EXAMPLES")
    else
        eval_job=$(sbatch --parsable --dependency=afterok:"$train_job" scripts/submit_evaluate.sh "results/$name")
    fi
