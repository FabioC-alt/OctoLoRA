...
(Section~\ref{sec:results}), which drifts over training rather than
sitting at a static value, is consistent with the gate tracking a
per-layer imbalance that Adam's per-parameter (not per-layer, not
$A$-vs-$B$-relative) statistics do not directly represent, but we regard
this as a hypothesis for future
